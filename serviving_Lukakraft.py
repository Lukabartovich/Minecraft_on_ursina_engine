...
tree_wood_texture = load_texture('assets/tree_block.png')
...
diamond_texture_1 = load_texture('assets/diamond_block.png')
...
class Voxel(Button):
    ...
    def input(self, key):
        ...
        global dirt_col
        global wood_col
        global tree_wood_col
        global leaves_col
        global diamond_1_col
        global diamond_2_col
        global tnt_col
        global slime_col
        global sand_col
        global gold_1_col
        ...
        if self.hovered:
            if key == 'left mouse down':
                ...
                if block_pick == 6: #water
                    ...
                    if collides == leaves_texture:
                            pos = voxel_interes.entity.position
                            destroy(voxel, delay=0.3)
                            destroy(voxel_interes.entity, delay=0.3)
                            voxel = Voxel(position=pos, texture=slime_texture)
                if block_pick == 7: #fire
                    voxel = Voxel(position=self.position + mouse.normal, texture=fire_texture)
                    voxel_interes = voxel.intersects()
                    collides = voxel_interes.entity.texture
                    if collides == tnt_texture:
                    ...
                    if collides == water_texture:
                        destroy(voxel, delay=0.5)
                        destroy(voxel_interes.entity, delay=0.5)
                    if collides == wood_texture or collides == tree_texture or collides == leaves_texture:
                        destroy(voxel_interes.entity, delay=0.3)
                        v_minus(voxel=voxel)
                    if collides == sand_texture:
                        pos = (voxel.x, voxel.y - 1, voxel.z)
                        destroy(voxel, delay=0.3)
                        destroy(voxel_interes.entity, delay=0.3)
                        voxel = Voxel(position=pos, texture=glass_texture)
                    if collides == diamond_texture_1:
                        pos = voxel_interes.entity.position
                        destroy(voxel, delay=0.3)
                        destroy(voxel_interes.entity, delay=0.3)
                        voxel = Voxel(position=pos, texture=diamond_texture_2)
                    if collides == gold_texture_1:
                        pos = voxel_interes.entity.position
                        destroy(voxel, delay=0.3)
                        destroy(voxel_interes.entity, delay=0.3)
                        voxel = Voxel(position=pos, texture=gold_texture_2)
                    if collides == dirt_texture:
                        pos = voxel_interes.entity.position
                        destroy(voxel, delay=0.3)
                        destroy(voxel_interes.entity, delay=0.3)
                        voxel = Voxel(position=pos, texture=brick_texture)
                if block_pick == 8: #leaves
                    if leaves_col > 0:
                            voxel = Voxel(position=self.position + mouse.normal, texture=leaves_texture)
                            leaves_col -= 1              
                if block_pick == 9: #diamond1
                    if diamond_1_col > 0:
                            voxel = Voxel(position=self.position + mouse.normal, texture=diamond_texture_1)
                            diamond_1_col -= 1             
                if block_pick == 10: #slime
                    if slime_col > 0:
                            voxel = Voxel(position=self.position + mouse.normal, texture=slime_texture)
                            slime_col -= 1
                            voxel_interes = voxel.intersects()
                            collides = voxel_interes.entity.texture
                            if collides == wood_texture:
                                pos = voxel_interes.entity.position
                                destroy(voxel, delay=0.3)
                                destroy(voxel_interes.entity, delay=0.3)
                                voxel = Voxel(position=pos, texture=tnt_texture)
                            if collides == diamond_texture_2:
                                pos = voxel_interes.entity.position
                                destroy(voxel, delay=0.3)
                                destroy(voxel_interes.entity, delay=0.3)
                                voxel = Voxel(position=pos, texture=gold_texture_1)
                            if collides == gold_texture_2:
                                pos = voxel_interes.entity.position
                                destroy(voxel, delay=0.3)
                                destroy(voxel_interes.entity, delay=0.3)
                                voxel = Voxel(position=pos, texture=diamond_texture_1)
                if block_pick == 11: #glass
                    if glass_col > 0:
                            voxel = Voxel(position=self.position + mouse.normal, texture=glass_texture)
                            glass_col -= 1
                if block_pick == 12: #sand
                    if sand_col > 0:
                            voxel = Voxel(position=self.position + mouse.normal, texture=sand_texture)
                            sand_col -= 1 
                if block_pick == 13: #portal
                    if portal_state == 0:
            ...
            if key == 'right mouse down':
                if self.texture == portal_texture:
                    ...
                    destroy(self)
                    wood_col += 1
                elif self.texture == tree_texture:
                    destroy(self)
                    tree_wood_col += 1
                elif self.texture == leaves_texture:
                    destroy(self)
                    leaves_col += 1
                elif self.texture == diamond_texture_1:
                    destroy(self)
                    diamond_1_col += 1
        ...
        for y in range(13):
            if y < stone_depth + 1:
                ...
                if x > 1 and z > 1 or x < voxeles and z < voxeles:
                    if stone_choice == 1:
                        voxel = Voxel(position = (x, y, z), texture=diamond_texture_1)
                    else:
